                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-md-12 d-flex justify-content-between">
                            ...
                            <a href="@if($loggedUser->role == 0) {{ route('system.additional.club-data.create-for', ['id' => $user->id ]) }} @else {{ route('system.additional.club-data.create') }} @endif" title=" {{ __('Unesite informacije o klubovima') }} ">
                        ...
                        <div class="col-md-12">
                            @foreach($user->clubDataRel as $clubData)
                                <a href="{{ route('system.additional.club-data.preview', ['id' => $clubData->id ?? '']) }}" title="{{ __('Uredite informacije za ') . ($clubData->clubRel->title ?? '') }}">
                                    <p class="m-0"> <small> {{ $clubData->seasonRel->value ?? '' }} - <b> {{ $clubData->clubRel->title ?? '' }} </b> </small> </p>
                                </a>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-12 d-flex justify-content-between">
                            ...
                            <a href="@if($loggedUser->role == 0) {{ route('system.additional.nat-team-data.create-for', ['id' => $user->id ]) }} @else {{ route('system.additional.nat-team-data.create') }} @endif " title=" {{ __('Unesite informacije o nastupima za reprezentaciju') }} ">
                                <small><i class="fas fa-plus"></i></small>
                            </a>
                        </div>
                        <div class="col-md-12">
                            @foreach($user->natTeamDataRel as $natTeamData)
                                <a href="{{ route('system.additional.nat-team-data.preview', ['id' => $natTeamData->id ?? '']) }}" title="{{ __('Uredite informacije za ') . ($natTeamData->countryRel->title ?? '') }}">
                                    <p class="m-0"> <small> {{ $natTeamData->seasonRel->value ?? '' }} - <b> {{ ucwords(strtolower($natTeamData->countryRel->name_ba)) ?? '' }} </b> </small> </p>
                                </a>
                            @endforeach
                        </div>
                    </div>
